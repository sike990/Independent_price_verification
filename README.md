...
In any financial institution, the Front Office (Traders) values its own portfolio. A core "control" function, VCS, must independently verify these prices to manage risk, prevent over-valuation, and ensure accurate reporting.

This dashboard automates the entire reconciliation and reporting process, allowing a manager to instantly identify the source and scale of pricing risks.
...
- **Dynamic MTM Variance Calculation:** Calculates the precise Mark-to-Market (MTM) dollar variance for every asset in the portfolio:  
  `(Official_Price - Trader_Price) * Quantity`
...
1. **Trader_Prices (Raw Data):** Source of truth for trader marks and descriptive data (Asset_Class, FVH_Level).
2. **Official_Prices (Raw Data):** Source of truth for independent VCS (market) prices.
3. **Portfolio_Holdings (Raw Data):** Source of truth for asset quantities.
4. **Control_Sheet (Calculation Engine):** The "brains" of the project. This sheet consolidates all data via XLOOKUP and performs all row-level calculations (e.g., MTM_Difference).
5. **Dashboard (Presentation Layer):** The final, high-level summary for management. Contains the Pivot Table, PivotChart, and Slicers.

## How to Use

1. Update the raw data in the first three sheets (`Trader_Prices`, `Official_Prices`, `Portfolio_Holdings`).
2. Navigate to the `Dashboard` sheet.
3. Right-click the Pivot Table and select **Refresh** to load the new data.
4. Use the **Asset_Class** and **FVH_Level** Slicers to interactively analyze the portfolio's pricing risk.
